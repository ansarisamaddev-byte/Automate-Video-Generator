...
DEFAULT_ASSETS_DIR = resolve_project_path("mindscribble_comic", "background_images")
...
def load_floating_sticker_clip(
    ...
    def make_frame(t):
        ...
        if t < pop_duration:
            ...
            pop_scale = 1.2 * progress - 0.2 * (progress ** 2)
...
def process_script_item(
    ...
    try:
        ...
        for idx, segment in enumerate(timeline):
            ...
            # Resolve path: supports direct file references or folders containing files
            asset_target_path = os.path.join(assets_dir, folder_name)
            media_files = []

            if os.path.isfile(asset_target_path):
                media_files = [asset_target_path]
            elif os.path.isdir(asset_target_path):
                valid_extensions = ("*.jpg", "*.jpeg", "*.mp4", "*.png", "*.webp")
                for ext in valid_extensions:
                    media_files.extend(glob.glob(os.path.join(asset_target_path, ext)))
            else:
                raise FileNotFoundError(f"Asset file or folder missing: {os.path.abspath(asset_target_path)}")

            if not media_files:
                raise FileNotFoundError(f"No media files found at target: {asset_target_path}")
        ...
        final_video_layers = [background] + all_sticker_clips
...
if __name__ == "__main__":
        ...
        "timeline": [
            {
                ...
                "background": {
                    "file": "bg_confident_character.png",
                    "effect": "zoom_in"
                },
                "stickers": [
                    {
    ...
    # 1. Normalize schema to match process_script_item interface expectations
    # Normalize schema
    normalized_timeline = []
    for seg in SCHEMA_DATA["timeline"]:
        bg_file = seg.get("background", {}).get("file", "")
        
        # FIX: Keep full filename (including extension) instead of stripping it
        folder_name = bg_file if bg_file else f"segment_{seg['segment_id']}"

        normalized_stickers = []
        for st in seg.get("stickers", []):
            st_copy = st.copy()
            st_copy["delay"] = st_copy.pop("delay_offset", 0.0)
            normalized_stickers.append(st_copy)
    ...
    normalized_script_data = {
        "script_title": SCHEMA_DATA["script_title"],
        "heading_text": SCHEMA_DATA["heading_text"],
        "timeline": normalized_timeline
    }

    # 2. Execute execution entry point
    output_path = process_script_item(
        script_data=normalized_script_data,
